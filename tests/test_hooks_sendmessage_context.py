"""Tests for `acp.adapters.hooks.sendmessage_context`."""
...
from acp.adapters.hooks.sendmessage_context import compress_message, main
...
class MainNoBlockTest(unittest.TestCase):
    def test_no_context_block_is_a_silent_noop(self) -> None:
        stdin = io.StringIO(json.dumps({
            "tool_input": {"to": "worker", "message": "plain message, no delimited block"},
            "session_id": "s1",
        }))
        buffer = io.StringIO()
        with patch("sys.argv", ["sendmessage_context.py", "--agent", "claude"]), \
    ...
    def test_missing_acp_home_is_a_silent_noop(self) -> None:
        stdin = io.StringIO(json.dumps({
            "tool_input": {"to": "worker", "message": "<acp-context>big</acp-context>"},
            "session_id": "s1",
        }))
        buffer = io.StringIO()
        with patch("sys.argv", ["sendmessage_context.py", "--agent", "claude"]), \
    ...
    def test_structured_message_is_a_silent_noop(self) -> None:
        # SendMessage's `message` may be a structured shutdown/plan-approval
        # object rather than plain text -- must be left completely untouched.
        stdin = io.StringIO(json.dumps({
            "tool_input": {
                "to": "worker",
                "message": {"type": "shutdown_request", "reason": "<acp-context>x</acp-context>"},
            },
            "session_id": "s1",
        }))
        buffer = io.StringIO()
...
class RealAcpTest(unittest.TestCase):
    ...
    def test_compress_message_leaves_undelimited_message_untouched(self) -> None:
        result = compress_message(self.client, "claude", "s1", "just tell the peer, no block here")
        self.assertIsNone(result)

    def test_compress_message_only_replaces_the_delimited_block(self) -> None:
        ...
        new_message = compress_message(self.client, "claude", "s1", message)
        self.assertIsNotNone(new_message)
        self.assertIn("Heads up.", new_message)
        self.assertIn("End of context.", new_message)
        self.assertIn("shrunk-block", new_message)
        self.assertNotIn(big_block, new_message)

    def test_small_block_pass_through_returns_none(self) -> None:
        message = "Heads up.\n<acp-context>tiny</acp-context>\nEnd."
        result = compress_message(self.client, "claude", "s1", message)
        self.assertIsNone(result)
    ...
    def test_main_emits_updated_input_for_a_large_block(self) -> None:
        ...
        message = f"Heads up.\n<acp-context>{big_block}</acp-context>\nEnd of context."
        stdin = io.StringIO(json.dumps({
            "tool_input": {"to": "worker", "message": message}, "session_id": "s1",
        }))
        buffer = io.StringIO()
        with patch("sys.argv", ["sendmessage_context.py", "--agent", "claude"]), \
             patch("sys.stdin", stdin), patch("sys.stdout", buffer), \
        ...
        self.assertIn("shrunk-block", updated_input["message"])
        self.assertIn("Heads up.", updated_input["message"])
        # everything outside `message` on the tool input is left untouched
        self.assertEqual(updated_input["to"], "worker")
